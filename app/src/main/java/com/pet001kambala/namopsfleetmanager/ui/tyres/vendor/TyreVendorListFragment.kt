...
class TyreVendorListFragment : AbstractTableFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        register_vendor.isEnabled = isAuthorized(AccessType.REG_TYRE_VENDOR)
        ...
        if(isAuthorized(AccessType.VIEW_TYRE_VENDOR)) {
            tyreModel.tyreVendorsList.observe(viewLifecycleOwner, Observer {
                it?.let { results ->
                    when (results) {
                        Results.Loading -> {
                            binding.vendorsCount = 1
                            showProgressBar("Loading tyre vendors...")
                        }
                        is Results.Success<*> -> {
                            endProgressBar()
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        menu.findItem(R.id.export_all_history).isEnabled = isAuthorized(AccessType.EXPORT_VENDOR)
    }
    @ExperimentalCoroutinesApi
